...
import { type ApiResponse, type PortfolioItem } from '@/types';
...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    async function loadPortfolio() {
      try {
        const response = await apiClient.get<ApiResponse<PortfolioItem[]>>('/portfolio/admin');
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await apiClient.post<ApiResponse<PortfolioItem>>('/portfolio', form);
  ...
  async function handleDelete(publicId: string) {
    await apiClient.delete(`/portfolio/${publicId}`);
    ...
    <main className="min-h-screen bg-[#07110f] px-6 py-10 text-white">
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#7fdbc0]">Marca</p>
            <h1 className="text-4xl font-semibold">Portafolio</h1>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <section className="rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-5 space-y-4">
              {loading ? (
                <p className="text-sm text-[#a8b8b3]">Cargando portafolio...</p>
              ) : items.length === 0 ? (
                <p className="text-sm text-[#a8b8b3]">Aun no hay casos publicados.</p>
              ) : (
                items.map((item) => (
                  <article key={item.publicId} className="rounded-2xl border border-white/10 bg-[#0b1614] p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-lg font-semibold">{item.title}</h3>
                        <p className="mt-1 text-sm text-[#a8b8b3]">{item.category} · {item.summary}</p>
                      </div>
                      <div className="flex flex-col items-end gap-2">
                        <span className="rounded-full bg-[#10201d] px-3 py-1 text-xs uppercase tracking-[0.18em] text-[#7fdbc0]">
                          {item.published ? 'publicado' : 'borrador'}
                        </span>
                        <button onClick={() => handleDelete(item.publicId)} className="text-xs text-red-300 hover:text-red-200">
                          Eliminar
                        </button>
                      </div>
                    </div>
